...
\tightlist
\item
  for all \(p\in P\), \(W'(\phi(p))=\phi(W(p))\);
\item
  for all \(c\in C\):
  ...
  \tightlist
  \item
    \(\sigma_{\psi(c)}=\sigma_c\),
  \item
    for all \(i\in\{0,\dotsc,\alpha(\sigma_c)-1\}\),
    \(\alpha(\psi(c))_i=\phi(\alpha(c)_i)\),
  \item
    for all \(j\in\{0,\dotsc,\pi(\sigma_c)-1\}\),
    \(\pi(\psi(c))_j=\phi(\pi(c)_j)\);
  \end{itemize}
\item
  \(L=L'\).
\end{itemize}
...
\item
  for all \(c\in C\) of symbol \(\sigma\),
  \(\tau(\pi(c))\vdash\tau(\alpha(c))\orth\);
\end{itemize}
